...
def mergenode(symbol, target_symbol, filetype, Frame):
    if filetype == "xml":
        for frame in symbol.findall(Frame):
            ...
            target_frame = target_symbol.find(
                ('./' + Frame + '[@framenum="{}"][@duration="{}"]').format(framenum, duration))
    ...
    # import
        ...
        for symbol in source_root.findall(Symbol):
            if symbol.get(symbolattr) == source_symbol_name:
                ...
                for frame in symbol.findall(Frame):
                    frame.set(frameattr, symbol_name + '-' +
                              frame.get(frameattr, "").split('-')[-1])
                root.append(symbol)
        tree.write(args[1])
    # rename
