...
def XmlToBuild(xml_path, output_path, endianstring="<"):
    ...
    outfile.write(struct.pack(endianstring + "I",
                              len(xml.getElementsByTagName("Frame"))))

    buildname = xml.getElementsByTagName(
        "Build")[0].getAttribute("name").encode("ascii")
    outfile.write(struct.pack(endianstring + "i" +
                  str(len(buildname)) + "s", len(buildname), buildname))
    ...
    for atlas in atlases:
        ...
        outfile.write(struct.pack(endianstring +
                                  "i" +
                                  str(len(atlas_name)) +
                                  "s", len(atlas_name), atlas_name.encode("ascii")))

    symbol_nodes = sorted(
        symbols, key=lambda x: strhash(
            x.getAttribute("name"), hash_dict))
    ...
    for hash_idx, name in hash_dict.items():
        ...
        outfile.write(struct.pack(endianstring + 'i' +
                      str(len(name)) + 's', len(name), name.encode("ascii")))
